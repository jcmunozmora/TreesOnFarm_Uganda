...
ifelse(cropID %in% c(880,970,990,950,960),"Trees for Timber/Fuelwood","Other Crops"))))
  ...
  crop_land_05_def_1 <- crop_land_05 %>% group_by(hhid,parcel_id,trees_a) %>% summarise(area_crop=sum(area_crop,na.rm=TRUE)) %>%
    ...
    ### ---> Rename all varibles
    setNames(c(names(.)[1], paste0("def1_",names(.)[-1])))

colnames(crop_land_05_def_1) <- c('hhid',
      'parcel_id','def1_fruit','def1_other_crop','def1_cash','def1_timber')
  ...
  crop_land_05_def_2 <- crop_land_05 %>% group_by(hhid,parcel_id,trees_b) %>% summarise(area_crop=sum(area_crop,na.rm=TRUE)) %>%
    ...
    ungroup() %>%
    dplyr::transmute(hhid=hhid,
      def2_fruit=def2_fruit,
      def2_other_crop=def2_other_crop,
      def2_cash=def2_cash,
      def2_timber=def2_timber,
      parcel_id=def2_parcel_id,
      parce_a_es=(def2_fruit+def2_other_crop+def2_timber+def2_cash))
  ...
  m_are_crop_05 <- crop_a_05 %>% left_join(land_05) %>% filter(!is.na(land_size)) %>%
    ...
    #### Build the shares
    transmute(hhid=hhid,
      sh_tof_w_b=treeonfarm_w_b/land_size,
      sh_tof_wo_b=treesonfarm_wo_b/land_size,
      sh_fruit_w_b=fruit_w_b/land_size,
      sh_fruit_wo_b=fruit_wo_b/land_size,
      sh_cash=cash/land_size,
      sh_timber=timber/land_size,
      land_size=land_size)
...
######################
  ...
  #####---- Land Size (Ownd)
  land_10_a <- read.dta13(paste0(path_work,"00_RawData/2010-11/Agriculture/AGSEC2A.dta")) %>% transmute(hhid=HHID,parcel_id=
    as.character(prcid),
    land_size=ifelse(is.na(a2aq4),a2aq5,a2aq4))

  #####---- Land Size (Use Rights)
  land_10_b <- read.dta13(paste0(path_work,"00_RawData/2010-11/Agriculture/AGSEC2B.dta")) %>% transmute(hhid=HHID,
    parcel_id=as.character(prcid),
    land_size=ifelse(is.na(a2bq4),a2bq5,a2bq4))

  #### Append Data Set
  land_10 <- rbind(land_10_a,land_10_b)

  crop_land_10 <- read.dta13(paste0(path_work,"00_RawData/2010-11/Agriculture/AGSEC4A.dta")) %>%
    transmute(hhid=HHID,
      parcel_id=as.character(prcid),
      plot_id=pltid,
      cropID=cropID,
      ...
      left_join(land_10) %>%
      mutate(share_crop=
        ifelse((share_crop/land_size)<=1 &
        (share_crop/land_size)>0,
        share_crop/land_size,1)) %>%
        filter(!is.na(share_crop)) %>% transmute(hhid,parcel_id,plot_id,
          cropID,
          area_crop=land_size*share_crop) %>%
    mutate(
    ##### First Classification - With Bananas
    trees_a=ifelse((cropID %in% c(710,700,750,760,770,741,742,744)),"Fruit Trees",
    ifelse(cropID %in% c(860,820,830,810,811,812),"Tree Cash Crops",
    ...
    ifelse(cropID %in% c(880,970,990,950,960),"Trees for Timber/Fuelwood","Other Crops"))))
    #### Building Aggregation for the first variable
    crop_land_10_def_1 <- crop_land_10 %>% group_by(hhid,parcel_id,trees_a) %>% summarise(area_crop=sum(area_crop,na.rm=TRUE)) %>%
    ### ---> Spread the data and rename
    spread(trees_a,area_crop,fill=0) %>%
    rename(fruit=`Fruit Trees`,other_crop=`Other Crops`,cash=`Tree Cash Crops`,timber=`Trees for Timber/Fuelwood`) %>%
    ### ---> Rename all varibles
  ...
  colnames(crop_land_10_def_1) <- c('hhid',
      'parcel_id','def1_fruit','def1_other_crop','def1_cash','def1_timber')


      #### Building Aggregation for the Second variable
      crop_land_10_def_2 <-crop_land_10 %>% group_by(hhid,parcel_id,trees_b) %>% summarise(area_crop=sum(area_crop,na.rm=TRUE)) %>%
        ### ---> Spread the data and rename
        spread(trees_b,area_crop,fill=0) %>%
        rename(fruit=`Fruit Trees`,other_crop=`Other Crops`,cash=`Tree Cash Crops`,timber=`Trees for Timber/Fuelwood`) %>%
        ### ---> Rename all varibles
      ...
      crop_a_10 <- crop_land_10_def_1 %>% inner_join(crop_land_10_def_2) %>% select(def1_fruit,parce_a_es,starts_with("def2_")) %>%
        ### Make the aggregations
        transmute(treeonfarm_w_b=(def1_fruit+def2_timber+def2_cash),treesonfarm_wo_b=(def2_fruit+def2_timber+def2_cash),fruit_w_b=def1_fruit,fruit_wo_b=def2_fruit,cash=def2_cash,timber=def2_timber,land_size_est=parce_a_es)
...
#############################################
        ...
        m_are_crop_10 <- crop_a_10 %>% left_join(land_10) %>% filter(!is.na(land_size)) %>%
          mutate(land_size=ifelse(land_size_est>land_size,land_size_est,land_size)) %>%
          #### Gen the aggregation by hiig
          group_by(hhid) %>%
...
######################
  ...
  colnames(crop_land_14_def_1) <- c('hhid',
                                    'parcel_id','def1_fruit','def1_other_crop','def1_cash')
  ...
  crop_land_14_def_2 <- crop_land_14 %>% group_by(hhid,parcel_id,trees_b) %>% summarise(area_crop=sum(area_crop,na.rm=TRUE)) %>%
    ...
    setNames(c(names(.)[1], paste0("def2_",names(.)[-1])))  %>% 
    ungroup() %>%
    mutate(parcel_id=def2_parcel_id,parce_a_es=(def2_fruit+def2_other_crop+def2_cash)) %>% 
    dplyr::select(-def2_parcel_id)
  
  #### Merge information

  crop_a_14 <- crop_land_14_def_1 %>% 
    inner_join(crop_land_14_def_2) %>% 
    select(def1_fruit,parce_a_es,starts_with("def2_")) %>%
    ### Make the aggregations
